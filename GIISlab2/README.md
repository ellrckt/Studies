# Лабораторная работа №2. Разработка графического редактора для построения линий второго порядка

## Цель работы

Разработать элементарный графический редактор, реализующий построение линий второго порядка: окружность, эллипс, гипербола и парабола. Редактор должен предоставлять возможность выбора кривой через меню и панель инструментов, а также иметь отладочный режим с пошаговым отображением процесса построения на дискретной сетке.

## Задачи

1.  Реализовать алгоритм построения окружности (например, алгоритм Брезенхема).
2.  Реализовать алгоритм построения эллипса.
3.  Реализовать алгоритм построения гиперболы.
4.  Реализовать алгоритм построения параболы.
5.  Создать графический интерфейс с использованием библиотеки Tkinter.
6.  Предусмотреть выбор кривой через меню и панель инструментов.
7.  Реализовать отладочный режим с пошаговым отображением построения кривой на дискретной сетке.
8.  Обеспечить возможность переключения между кривыми в отладочном режиме.

## Используемые алгоритмы

### 1. Алгоритм построения окружности (алгоритм Брезенхема)

Алгоритм Брезенхема для окружностей использует целочисленные вычисления для определения, какой пиксель выбрать на каждом шаге, минимизируя расстояние между идеальной окружностью и растровой окружностью. Он использует 8-стороннюю симметрию для отрисовки сразу 8 пикселей за одну итерацию.

#### Описание алгоритма

1.  Определяются параметры: `center_x`, `center_y` (координаты центра), `radius` (радиус).
2.  Инициализация: `x = 0`, `y = radius`, `decision_parameter = 3 - 2 * radius`.
3.  Цикл выполняется, пока `x <= y`.
4.  Для каждой итерации:
    *   Отрисовка 8 симметричных точек:
        ```
        drawPixel(center_x + x, center_y + y)
        drawPixel(center_x - x, center_y + y)
        drawPixel(center_x + x, center_y - y)
        drawPixel(center_x - x, center_y - y)
        drawPixel(center_x + y, center_y + x)
        drawPixel(center_x - y, center_y + x)
        drawPixel(center_x + y, center_y - x)
        drawPixel(center_x - y, center_y - x)
        ```
